...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (tg) {
      // 1. Mở rộng toàn màn hình
      tg.expand();

      // 2. Báo cho Telegram biết App đã load xong để tắt animation chờ
      tg.ready();

      // 3. Khóa thao tác vuốt dọc (Vertical Swipe) làm đóng app ngoài ý muốn
      if (tg.isVersionAtLeast('6.1')) {
        tg.disableVerticalSwipes();
      }

      // 4. Đồng bộ màu nền của Telegram để tránh viền trắng
      tg.setHeaderColor('#000000'); // Trùng với màu bg-black của bạn
  ...
  const endGame = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/submit-game', { // Đảm bảo tên file của bạn là submit-game.js
        method: 'POST',
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("Data nhận được từ server:", data); // DEBUG: Mở F12 Console để xem có nhận được leaderboard không
        setLeaderboardData(data.leaderboard || []);
        setUserRank(data.user_rank);
        setUserFinalScore(data.final_score); // Đảm bảo key này khớp với API trả về
      }
    } catch (err) {
                  ...
                  <div key={idx} className="flex justify-between items-center bg-black/40 p-3 rounded-xl border border-gray-800/50">
                    ...
                    <div className="text-right">
                      <div className="font-mono text-yellow-500 text-sm">
                        {Number(player.best_score || 0).toLocaleString()}
                      </div>
                    </div>
                  </div>
                ))}
